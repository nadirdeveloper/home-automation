...
export class RoomDevices extends Component {
  ...
  toggleDeviceSwitch = deviceId => {
    var dataToSend = deviceId;
    // dataToSend.roomName = this.props.history.location.pathname.split("/")[2];
    this.props.toggleDeviceSwitch(dataToSend);
  };
            ...
            return (
              <div key={deviceId} className={classes.Column}>
                <Device
                  deviceId={deviceId}
                  device={deviceData}
                  onToggleDeviceSwitch={() => this.toggleDeviceSwitch({deviceData,deviceId})}
